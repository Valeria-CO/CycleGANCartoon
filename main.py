...
for epoch in range(0):
    ...
    D_X.train()
    D_Y.train()
    loop = tqdm(data_train, desc=f"Эпоха {epoch}/{100}", leave=False)
    for real_X, real_Y in loop:
        ...
        # loss_D_Y_real = adversarial_loss(D_Y(real_Y_noisy), torch.full_like(D_Y(real_Y), 0.9))
        # loss_D_Y_fake = adversarial_loss(D_Y(fake_Y_noisy), torch.zeros_like(D_Y(fake_Y)))
        ...
        # loss_D_X_real = adversarial_loss(D_X(real_X_noisy), torch.full_like(D_X(real_X), 0.9))
        # loss_D_X_fake = adversarial_loss(D_X(fake_X_noisy), torch.zeros_like(D_X(fake_X)))
...
img = Image.open('002.jpg').convert('RGB')
